...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        total_weight = float(sum(self.values()))
        if total_weight == 0:
            raise ValueError("Cannot sample from an empty distribution")

        # Normalize weights to probabilities
        probabilities = {key: value / total_weight for key, value in self.items()}
        
        # Draw a random sample based on probabilities
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        #Check is weights are zero
        if distribution.total() == 0:
            self.initializeUniformly(gameState)
        else:
            #Resample particales based on weights
